...
class Users extends Table {
  ...
  TextColumn get role =>
      text()(); // 'super_admin', 'secretary', 'it_admin', 'servant'
...
class Attendances extends Table {
  ...
  TextColumn get serviceType =>
      text()(); // 'Sunday', 'Tuesday', 'Special Event'
...
class SyncQueue extends Table {
  ...
  TextColumn get entityType =>
      text()(); // 'contact', 'attendance', 'scenario', 'scenario_task'
...
class Locations extends Table {
  IntColumn get id => integer().autoIncrement()();
  TextColumn get value => text().withLength(
    min: 1,
    max: 50,
  )(); // Unique identifier (e.g., 'kanana')
  TextColumn get displayName =>
      text().withLength(min: 1, max: 100)(); // Display name (e.g., 'Kanana')
  IntColumn get colorValue =>
      integer().withDefault(const Constant(0xFFF44336))(); // Color as int
  BoolColumn get isActive => boolean().withDefault(const Constant(true))();
  IntColumn get sortOrder =>
      integer().withDefault(const Constant(0))(); // For manual sorting
  DateTimeColumn get createdAt => dateTime().withDefault(currentDateAndTime)();
}
...
class AttendanceWithContact {
  ...
  AttendanceWithContact({required this.attendance, this.contactName});
...
class AppDatabase extends _$AppDatabase {
  ...
  Future<void> _seedDefaultLocations() async {
    final defaultLocations = [
      ...
      LocationsCompanion.insert(
        value: 'majaneng',
        displayName: 'Majaneng',
        colorValue: const Value(0xFFE91E63),
        sortOrder: const Value(2),
      ),
      LocationsCompanion.insert(
        value: 'mashemong',
        displayName: 'Mashemong',
  ...
  Future<List<ContactEntity>> getContactsByTag(String tag) {
    return (select(contacts)..where(
          (t) => t.metadata.contains('"$tag"') & t.isDeleted.equals(false),
        ))
        .get();
  }
  ...
  Future<List<ContactEntity>> searchContacts(String query) {
    ...
    return (select(contacts)..where((t) {
          if (normalizedPhone != null) {
            ...
            return (t.name.contains(trimmedQuery) |
                    t.phone.contains(normalizedPhone)) &
            ...
            return (t.name.contains(trimmedQuery) |
                    t.phone.contains(searchDigits)) &
                t.isDeleted.equals(false);
          } else {
            // Regular search by name or phone
            return (t.name.contains(trimmedQuery) |
                    t.phone.contains(trimmedQuery)) &
                t.isDeleted.equals(false);
          }
        }))
        .get();
  ...
  Future<int> updateContactFields({
    ...
    final companion = ContactsCompanion(
      ...
      optOutWhatsapp: optOutWhatsapp != null
          ? Value(optOutWhatsapp)
          : const Value.absent(),
  ...
  Future<int> softDeleteContact(int id) {
    return (update(contacts)..where((t) => t.id.equals(id))).write(
      const ContactsCompanion(isDeleted: Value(true)),
    );
  }
  ...
  Future<List<AttendanceWithContact>> getAttendancesWithContactsByDateRange(
    ...
    final query =
        select(attendances).join([
            leftOuterJoin(
              contacts,
              contacts.id.equalsExp(attendances.contactId),
            ),
          ])
          ..where(
            attendances.serviceDate.isBiggerOrEqualValue(from) &
  ...
  Future<int> deleteAttendance(int id) =>
      (delete(attendances)..where((t) => t.id.equals(id))).go();
  ...
  Future<int> deleteScenario(int id) =>
      (update(scenarios)..where((t) => t.id.equals(id))).write(
        const ScenariosCompanion(isDeleted: Value(true)),
      );
  ...
  Future<List<ContactEntity>> getContactsWithSuccessfulSync() async {
    ...
    return allContacts
        .where((contact) => !pendingSyncLocalIds.contains(contact.id))
        .toList();
  }
  ...
  Future<LocationEntity?> getLocationByValue(String value) {
    return (select(
      locations,
    )..where((t) => t.value.equals(value))).getSingleOrNull();
  }
  ...
  Future<int> deactivateLocation(int id) {
    return (update(locations)..where((t) => t.id.equals(id))).write(
